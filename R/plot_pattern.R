...
#' location, age. plot_trends creates plots of the estimates over some unit of time comparing the
#' two sets of results. This could be used for time trends across years, or age patterns. The 
#' options for faceting or coloring allow the user to customize the plot to maximize utility. This 
#' function assumes that the two sets of estimates are in two different columns, e.g. old_mean and
#' new_mean, and uses pivot_longer from tidyr to reshape the data long to make plotting more
...
#' @param y1_var `character()` Variable name for first set of results/estimates, e.g. the previous
#' or old set for comparison. Required!
...
#' The "grid" option will produce a plot using facet_grid. Required!
...
#' @param my_labs Vector. This vector would contain the labels/names you want to use for the two 
#' different sets of estimates. If left blank this will default to the column names for y1_var 
#' and y2_var. 
